...
from PyQt5.QtWidgets import (
    ...
    QFileDialog, QLabel, QProgressBar, QSpinBox, QComboBox
)
...
class EOSLogsTab(QtWidgets.QWidget):
    ...
    def _setup_ui(self):
        ...
        self.load_btn = QPushButton("Load EOS Logs")
        ...
        # Data table (columns will be determined dynamically based on the loaded EOS file)
    ...
    def _load_file(self):
        ...
        try:
            self.main_window.begin_busy("Loading EOS log files...")
            self.main_window.status_bar.showMessage("Loading EOS log files...")
            ...
            for index, filepath in enumerate(filepaths, start=1):
                self.main_window.status_bar.showMessage(
                    f"Loading EOS log files... ({index}/{len(filepaths)}) {os.path.basename(filepath)}"
                )
            ...
            self.file_label.setText(
                f"{imported_files} imported, {skipped_files} skipped ({imported_entries} entries)"
            )
            self.current_page = 0
            ...
            self.main_window.status_bar.showMessage(
                f"Loaded {imported_entries} EOS entries from {imported_files} file(s), skipped {skipped_files}"
            )
        except Exception as e:
            self.main_window.status_bar.showMessage("Failed to load EOS log files")
    ...
    def _populate_tx_combo(self):
        ...
        for tx in self.parser.get_transactions(station_id=station_id):
            label = f"{tx['transaction_id']} ({tx.get('start_time','')[:16]} - {tx.get('stop_time','')[:16]})"
    ...
    def _display_page(self):
        ...
        if self.hide_empty_checkbox.isChecked():
            messages = [m for m in messages if not self._is_empty_row(m)]
        ...
        # Determine table columns dynamically based on the loaded data keys
        columns = []
        for msg in messages:
            for key in msg.keys():
                if key == "raw_data":
        ...
        # Prefer standard columns at the front (if present)
        ...
        ordered_columns += [c for c in columns if c not in ordered_columns]
        ...
        for row_idx, msg in enumerate(messages):
            for col_idx, col_name in enumerate(columns):
                self.table.setItem(row_idx, col_idx, QTableWidgetItem(str(msg.get(col_name, ""))))
    ...
    def _is_empty_row(self, row: dict) -> bool:
        """Detect rows where every data column is empty/None."""
        # Ignore identifying fields which are constant per import
