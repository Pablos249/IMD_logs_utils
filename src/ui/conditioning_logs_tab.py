"""UI for conditioning logs module."""
...
class ConditioningLogsTab(QtWidgets.QWidget):
    ROWS_PER_PAGE = 100

    def __init__(self, main_window):
        super().__init__()
        self.main_window = main_window
        self.parser = ConditioningLogParser()
        self.current_page = 0
        self._setup_ui()
    ...
    def _setup_ui(self):
        ...
        self.load_btn = QPushButton("Load Conditioning Logs")
        self.load_btn.clicked.connect(self._load_file)
        file_layout.addWidget(QLabel("Conditioning Log File:"))
    ...
    def _load_file(self):
        ...
        filepaths, _ = QFileDialog.getOpenFileNames(
            self,
            "Open Conditioning Log Files",
            "conditioning",
            "Conditioning Log Files (*.log *.txt);;All Files (*.*)",
        )
        if not filepaths:
            return

        try:
            self.main_window.begin_busy("Loading conditioning log files...")
            self.main_window.status_bar.showMessage("Loading conditioning log files...")
            ...
            for index, filepath in enumerate(filepaths, start=1):
                self.main_window.status_bar.showMessage(
                    "Loading conditioning log files... "
                    f"({index}/{len(filepaths)}) {os.path.basename(filepath)}"
                )
            ...
            self.main_window.status_bar.showMessage(
                "Loaded "
                f"{imported_lines} conditioning lines from {imported_files} file(s), skipped {skipped_files}"
            )

            self.progress_bar.setValue(100)
            QtCore.QTimer.singleShot(1000, lambda: self.progress_bar.setVisible(False))
        except Exception as exc:
            self.main_window.status_bar.showMessage("Failed to load conditioning log files")
    ...
    def _on_parse_progress(self, current: int, total: int):
        if total > 0:
            ...
            self.main_window.status_bar.showMessage(f"Loading conditioning log file... {percent}%")
    ...
    def _on_station_switched(self):
        self._begin_busy("Refreshing conditioning view...")
        try:
            self.current_page = 0
            self.file_label.setText("")
            self._populate_file_combo()
            ...
            self._end_busy("Conditioning view refreshed.")

    def refresh_for_current_station(self):
        self._on_station_switched()

    def _on_filter_changed(self):
        self._begin_busy("Refreshing conditioning data...")
        try:
            self.current_page = 0
            self._reset_pagination()
            self._display_page()
        finally:
            self._end_busy("Conditioning data refreshed.")
    ...
    def _prev_page(self):
        if self.current_page > 0:
            self._begin_busy("Loading conditioning page...")
            try:
                self.current_page -= 1
                self.page_spinbox.blockSignals(True)
                ...
                self._end_busy("Conditioning page loaded.")
    ...
    def _delete_file_logs(self):
        ...
        reply = QtWidgets.QMessageBox.question(
            self,
            "Confirm Deletion",
            f"Delete all conditioning entries imported from '{file_name}'?",
            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
        )
        if reply != QtWidgets.QMessageBox.Yes:
            return

        self._begin_busy("Deleting conditioning data...")
        try:
            ...
            self._end_busy("Conditioning data deleted.")
