...
class ConditioningLogParser:
    ...
    def _init_db(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_conditioning_measurements_entry "
            "ON conditioning_measurements(entry_id)"
        )
        self.db_connection.commit()

    def _migrate_measurements_storage(self):
        cursor = self.db_connection.cursor()
        cursor.execute("PRAGMA table_info(conditioning_measurements)")
        existing_columns = {row[1] for row in cursor.fetchall()}
        needs_rebuild = "raw_line" in existing_columns or "entry_id" not in existing_columns
        if not needs_rebuild:
            return

        cursor.execute("ALTER TABLE conditioning_measurements RENAME TO conditioning_measurements_legacy")
        cursor.execute(
            """
            CREATE TABLE conditioning_measurements (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                station_id INTEGER,
                file_name TEXT NOT NULL,
                file_hash TEXT NOT NULL,
        ...
        cursor.execute(
            ...
            SELECT
                ...
                (
                    SELECT MIN(entry.id)
                    FROM conditioning_log_entries AS entry
                    WHERE entry.station_id IS legacy.station_id
                      ...
                      AND (
                          NOT EXISTS (
                              SELECT 1
                              FROM pragma_table_info('conditioning_measurements_legacy')
                              WHERE name = 'raw_line'
                          )
                          OR entry.raw_line = legacy.raw_line
                      )
                ) AS entry_id
            FROM conditioning_measurements_legacy AS legacy
            """
        )
        cursor.execute("DROP TABLE conditioning_measurements_legacy")
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_conditioning_measurements_series "
            "ON conditioning_measurements(station_id, metric_name, metric_scope, timestamp)"
        )
